...
static void gll_set_default(global_settings *gs) {
    if (gs) {
        ...
        gs->snp_list_file = NULL; gs->pl = NULL; gs->is_target = 0; gs->targets = NULL;
...
static int check_args(global_settings *gs) {
    ...
    */
    ...
    if (gs->snp_list_file) {
        if (NULL == (gs->pl = msnplist_init())) {
            fprintf(stderr, "[E::%s] could not init snp list.\n", __func__); 
            return -2;
        }
    }
    if (gs->umi_tag) {
...
static int snp2regidx(global_settings *gs) {
    int ret;
    snp1_t *snp = NULL;
    biallele_t **ale = NULL;
    if (NULL == (ale = (biallele_t**) calloc(1, sizeof(biallele_t*)))) { goto fail; }
    if (NULL == (gs->targets = regidx_init(NULL, NULL, regidx_payload_free, sizeof(biallele_t*), NULL))) { goto fail; } 
    if (NULL == (snp = snp1_init())) { goto fail; }
    while (msnplist_next(gs->pl, snp)) {
        if (NULL == (*ale = biallele_init())) { goto fail; }
        (*ale)->ref = snp->data->ref; 
        (*ale)->alt = snp->data->alt; 
        if (regidx_push(gs->targets, snp->chrom, snp->chrom + strlen(snp->chrom) - 1, snp->data->pos, snp->data->pos, ale) < 0) {
    ...
    if (gs.snp_list_file) {
        ...
        if (gs.is_target) {
            ...
            if (get_snplist(gs.snp_list_file, gs.pl, &ret, print_skip_snp) <= 0 || ret < 0) {
                ...
                n = msnplist_size(gs.pl);
                fprintf(stderr, "[I::%s] pileuping %ld candidate variants ...\n", __func__, n); 
            }
            if (snp2regidx(&gs) < 0) {
                fprintf(stderr, "[E::%s] failed to construct regidx from snplist.\n", __func__);
                print_time = 1; goto fail;
            }
            msnplist_destroy(gs.pl); gs.pl = NULL;
